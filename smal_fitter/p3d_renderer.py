...
class Renderer(torch.nn.Module):
    def __init__(self, image_size, device):
        ...
        self.mesh_color = (
            torch.FloatTensor(config.MESH_COLOR).to(device)[None, None, :] / 255.0
        )
        ...
        self.color_renderer = MeshRenderer(
            rasterizer=MeshRasterizer(
                cameras=self.cameras, raster_settings=raster_settings_color
            ),
    ...
    def forward(self, vertices, points, faces, render_texture=False):
        ...
        screen_size = (
            torch.ones(vertices.shape[0], 2).to(vertices.device) * self.image_size
        )
        # proj_points = self.cameras.transform_points_screen(points, screen_size)[
        #     :, :, [1, 0]
        # ]

        proj_points = self.cameras.transform_points_screen(points, image_size=(self.image_size, self.image_size))[:, :, [1,0]]
